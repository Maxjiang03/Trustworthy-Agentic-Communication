...
Create `docs/workplan/archive/g8g5/`, copy this file into it byte-for-byte, add a `MANIFEST.md` with its SHA-256 and the exact label used for g1: **"retrospective records — NOT pre-registration evidence."** Remove the working copy from the repo root in the same commit.
