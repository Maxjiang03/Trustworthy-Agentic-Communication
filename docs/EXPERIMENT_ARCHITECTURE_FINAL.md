...
|---|-----------|--------|------|
| IA-1 | The chosen Python Biscuit library exists, is maintainable, and exposes append-block attenuation + root-public-key verification with a stable API | **[UNVERIFIED-IA]** (FAQ: format production-used but **not formally audited**; Python-library maturity open) | G-1 |
...
|------|------|----------------|--------|
| **G-1** | Import the Python Biscuit library; mint `C_0`, append one block → `P_1`, verify against root public key | Round-trips and verifies; API stable enough to script | IA-1; whole capability track |
