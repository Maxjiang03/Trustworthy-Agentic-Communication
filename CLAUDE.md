...
## Current phase
Experimental apparatus, pilot corpus only. Nine smoke gates pass (`smoke/README.md`); the golden thread runs end to end — Supervisor → A2A hop (behind a port, ADR 0020) → Specialist → MCP tool call over the frozen `Ω`. Since 2026-07-31 **four arms are built** — `B0`, `B2-exchange-task`, `B-cap`, `B3` — over a four-scenario pilot corpus (`gt-benign`, `gt-f1-root`, `gt-f1-terminal`, `gt-f1-chain-tamper`); the other five ladder arms and every §E.6 ablation are not built. Rows 4/6/10 are frozen as `H(Λ)` (ADR 0022, composition amended by ADR 0023), so 5 of 11 `frozen_parameters` rows are set. **G-13 was adjudicated on 2026-07-31** — matched per-hop authority, PASS over the arms that exist, with the `B2-exchange-task-DPoP` and `B3⁺` limbs **open** and `IA-3` untouched. Gates G-3, G-9, G-10, G-12, G-14, G-15 remain. Pre-registration still a stub; `fixtures/confirmatory/` still empty; **no timing number measured or reported** (rows 1–2 UNSET).
